...
async function createChapterBySubject(req,res,next){
    try {
        ...
        const { title, type, content, attachments, linkVideo, quizzes, createBy } = req.body;
        const newChapter = await DbChapter.create({title, content, attachments, linkVideo, type, subjectId, quizzes, createBy})
...
async function updateChapter(req,res,next){
  try {
    ...
    const { title, type, content, linkVideo, quizzes } = req.body;

    const updateSubject = await ChapterRepo.updateChapter({ chapterId, title, type, content, linkVideo, quizzes });
